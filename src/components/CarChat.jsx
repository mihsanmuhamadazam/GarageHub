      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          ...
          <p className="text-midnight-300 text-lg max-w-md">
            Leave messages, requests, and coordinate with other drivers seamlessly.
          </p>
        </div>
...
function MessageBubble({ message, user, currentUser, onDelete, index }) {
  ...
  return (
    <div 
      className={`flex gap-3 animate-slideUpFade ${isOwn ? 'flex-row-reverse' : ''}`}
      style={{ animationDelay: `${index * 50}ms` }}
    >
      <div 
        className="w-10 h-10 rounded-full flex items-center justify-center text-xl shrink-0 transition-transform duration-300 hover:scale-110"
        style={{ backgroundColor: user?.color + '30', boxShadow: `0 4px 15px ${user?.color}30` }}
      >
        {user?.avatar}
      </div>
      ...
      <div className={`max-w-[70%] ${isOwn ? 'items-end' : 'items-start'}`}>
        ...
        <div className={`relative group rounded-2xl p-4 transition-all duration-300 hover:scale-[1.02] ${
          isOwn 
            ? 'bg-gradient-to-br from-ember-500/30 to-ember-600/20 rounded-tr-sm border border-ember-500/20' 
...
function CarSelector({ cars, selectedCarId, onSelect }) {
  return (
...
export default function CarChat() {
  const { messages, cars, users, currentUser, addMessage, deleteMessage } = useStore()
              ...
              <div className="flex-1 flex items-center justify-center h-full min-h-[200px]">
                <div className="text-center animate-fadeIn">
                  ...
                  <p className="text-sm text-midnight-400 max-w-xs">
                    Start a conversation about this vehicle. Leave notes, requests, or questions for other drivers.
                  </p>
                </div>
              </div>
            )}
